<?php
/**
 * Fixed Version of admin/dashboard.php
 * 
 * This file fixes the column name mismatch issue while preserving all functionality
 */
...
// Get revenue stats - FIXED queries to use 'amount' instead of 'total_amount'
$stmt = $conn->prepare("SELECT SUM(amount) as total_revenue FROM projects");
$stmt->execute();
$totalRevenue = $stmt->fetch(PDO::FETCH_ASSOC)['total_revenue'] ?? 0;

$stmt = $conn->prepare("SELECT SUM(amount) as paid_revenue FROM projects WHERE payment_status = 'paid'");
$stmt->execute();
$paidRevenue = $stmt->fetch(PDO::FETCH_ASSOC)['paid_revenue'] ?? 0;

$stmt = $conn->prepare("SELECT SUM(amount) as pending_revenue FROM projects WHERE payment_status = 'pending'");
$stmt->execute();
...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <div>
                <a href="add-client.php" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded inline-flex items-center mr-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <ul class="divide-y divide-gray-200">
                        <?php foreach ($recentClients as $client): ?>
                            <li class="py-3 flex justify-between items-center">
                                <div>
                                    <p class="text-sm font-medium text-gray-900"><?php echo htmlspecialchars($client['name']); ?></p>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <?php
                // Get recent payments
                $stmt = $conn->prepare("SELECT pa.*, pr.name as project_name 
                                        FROM payments pa
